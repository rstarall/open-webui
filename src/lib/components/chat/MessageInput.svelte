<script lang="ts">
	...
	// Check if any selected model has force_advanced_pdf enabled
	let forceAdvancedPdf = false;
	$: forceAdvancedPdf = (atSelectedModel?.id ? [atSelectedModel.id] : selectedModels).some((modelId) => {
		const model = $models.find((m) => m.id === modelId);
		return model?.info?.meta?.capabilities?.force_advanced_pdf === true;
	});
	...
	const uploadFileHandler = async (file, fullContext: boolean = false) => {
		...
		if (!$temporaryChatEnabled) {
			try {
				...
				// Add advanced PDF parsing flag if enabled or forced by model
				if ((advancedPdfEnabled || forceAdvancedPdf) && file.type === 'application/pdf') {
					metadata = {
						...metadata,
									...
									<div class="ml-1 self-end flex items-center flex-1 max-w-[80%]">
										...
										{#if $_user && (showToolsButton || (toggleFilters && toggleFilters.length > 0) || showWebSearchButton || showImageGenerationButton || showCodeInterpreterButton)}
											...
											<div class="flex gap-1 items-center overflow-x-auto scrollbar-none flex-1">
												...
												{:else}
													<!-- Show indicator when force_advanced_pdf is enabled -->
													<Tooltip content={$i18n.t('高级PDF解析已由模型强制启用')} placement="top">
														<div
															class="px-2 @xl:px-2.5 py-2 flex gap-1.5 items-center text-sm rounded-full text-sky-500 dark:text-sky-300 bg-sky-50 dark:bg-sky-200/5"
														>
															<svg
																xmlns="http://www.w3.org/2000/svg"
																viewBox="0 0 20 20"
															...
															>
																...
																/>
																<path
																	d="M10.5 5a.5.5 0 00-1 0v2.5H7a.5.5 0 000 1h2.5V11a.5.5 0 001 0V8.5H13a.5.5 0 000-1h-2.5V5z"
																	fill="#fff"
																	opacity="0.5"
																/>
															</svg>
