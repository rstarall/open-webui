...
export const getGlobalPinnedModels = async (token: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/global/pinned-models`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const setGlobalPinnedModels = async (token: string, models: string[]) => {
	let error = null;
